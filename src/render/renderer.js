...
export function createRenderer(canvas) {
  ...
  function render(scene, view) {
    ...
    // ---- fili: solo dove servono a dire "guarda quanto è ambiguo"
    const wires = scene.objects.filter((o) => o.wire && o.gpu.wireVao);
    if (wires.length) {
      setCommon(wire, ctx);
      ...
      for (const obj of wires) {
        ...
        gl.uniform3f(wire.u.uTint, ...obj.tint);
        gl.uniform1f(wire.u.uOpacity, obj.opacity ?? 1);
